		...
		<c:choose>
		    <c:when test="${paidFines[0] != null}">
		      	<table id="dataTable" style="background-color:white" class="table table-striped table-bordered table-hover table-condensed">
			        ...
			        <tbody>
				    	<c:forEach var="paidFine" items="${paidFines}">
							<tr>
								<td><c:out value="${paidFine.id}"/></td>
								<td><fmt:formatDate pattern="dd.MM.yyyy HH:mm ч." value="${paidFine.date}" /></td>
								<td><c:out value="${paidFine.vehicleRegNum}"/></td>
								<td><c:out value="${paidFine.kindOfViolation}"/></td>
								<td><c:out value="${paidFine.parkingNumber}"/></td>
								<td><c:out value="${paidFine.description}"/></td>
								<td><fmt:formatDate pattern="dd.MM.yyyy HH:mm ч." value="${paidFine.finePaymentInfo.paidOnDate}" /></td>
								<td><c:out value="${paidFine.finePaymentInfo.paidBy}"/></td>
								<td><c:out value="${paidFine.finePaymentInfo.paidAmount}"/></td>
							</tr>
						</c:forEach>
			        </tbody>
				</table>     
